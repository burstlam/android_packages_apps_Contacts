...
public class CallDetailHistoryAdapter extends BaseAdapter {
	...
	public CallDetailHistoryAdapter(Context context,
			LayoutInflater layoutInflater, CallTypeHelper callTypeHelper,
			PhoneCallDetails[] phoneCallDetails, boolean showVoicemail,
			boolean showCallAndSms, View controls) {
		mContext = context;
	...
	/** shutao 2012-10-15 Open the Click */
	// @Override
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		// voicemail.
		// View voicemailContainer =
		// header.findViewById(R.id.header_voicemail_container);
		// voicemailContainer.setVisibility(mShowVoicemail ? View.VISIBLE :
		// View.GONE);
		// // Call and SMS controls are only shown in the main UI if there is a
		// known number.
		// View callAndSmsContainer =
		// header.findViewById(R.id.header_call_and_sms_container);
		// callAndSmsContainer.setVisibility(mShowCallAndSms ? View.VISIBLE :
		// View.GONE);
		...
		final View result = convertView == null ? mLayoutInflater.inflate(
				R.layout.call_detail_history_item, parent, false) : convertView;

		PhoneCallDetails details = mPhoneCallDetails[position /*- 1*/];
		CallTypeIconsView callTypeIconView = (CallTypeIconsView) result
				.findViewById(R.id.call_type_icon);
		TextView callTypeTextView = (TextView) result
				.findViewById(R.id.call_type_text);
		/** shutao 2012-10-17 */
		TextView dateView = (TextView) result.findViewById(R.id.shendu_date);
		TextView timeView = (TextView) result.findViewById(R.id.shendu_time);
		TextView durationView = (TextView) result.findViewById(R.id.duration);
		TextView shendu_number = (TextView) result
				.findViewById(R.id.shendu_call_detail_number);
		TextView shendu_Attribution = (TextView) result
				.findViewById(R.id.shendu_call_detail_attribution);
		/** shutao 2012-10-18 add Attribution */
		String city = PhoneLocation.getCityFromPhone(details.number.toString());

		if (city != null) {
			shendu_Attribution.setText(city);
		} else {
			shendu_Attribution.setText("");
		}
		...
		CharSequence dateValue = DateUtils.formatDateRange(mContext,
				details.date, details.date, DateUtils.FORMAT_SHOW_DATE 
				/* * | * DateUtils * .* FORMAT_SHOW_WEEKDAY * |* DateUtils* .
				* FORMAT_SHOW_YEAR */);
		CharSequence timeValue = DateUtils.formatDateTime(mContext,
				details.date, DateUtils.FORMAT_SHOW_TIME);
		dateView.setText(dateValue.toString().replaceAll(" ", ""));
		timeView.setText(timeValue.toString().replace(" ", ""));
		// Set the duration
