 ...
 */
/*package*/ class ShenduCallLogAdapter extends ShenduGroupingListAdapter
        implements ViewTreeObserver.OnPreDrawListener, ShenduCallLogGroupBuilder.GroupCreator {
    ...
    private final ShenduCallLogGroupBuilder mCallLogGroupBuilder;
...
//        }
    ...
    ShenduCallLogAdapter(Context context, CallFetcher callFetcher,
        ...
        mCallLogGroupBuilder = new ShenduCallLogGroupBuilder(this);
    }
...
//                        mHandler.sendEmptyMessage(REDRAW);
    ...
    protected View newGroupView(Context context, ViewGroup parent) {
        ...
        View view = inflater.inflate(R.layout.shendu_call_log_list_item, parent, false);
    ...
    private void bindView(View view, Cursor c) {
    	...
    	String common_name = c.getString(CallLogQuery.CACHED_NAME);
...
//        views.bottomDivider.setVisibility(isLastOfSection(c) ? View.GONE : View.VISIBLE);
        views.listHeaderTextView.setVisibility(View.GONE);
    	 count = mCallLogGroupBuilder.getChildCallLogName().get(common_name) == null ? 0 : mCallLogGroupBuilder.getChildCallLogName()
				.get(common_name).types.size();
        ...
        views.secondaryActionView.setTag(shenduGetCallDetailIntentProvider(
				count, common_name));
        ...
        final int[] callTypes = getCallTypes(common_name, count);
     ...
     */
    private int[] getCallTypes(String name, int count) {
    	int[] callTypes = new int[count];
		for (int index = 0; index < count; ++index) {
			callTypes[index] = mCallLogGroupBuilder.getChildCallLogName().
					get(name).types.get(index);
		}
		return callTypes;
    }
	...
	public IntentProvider shenduGetCallDetailIntentProvider(final int count,
			final String name) {
		return new IntentProvider() {
			...
			public Intent getIntent(Context context) {
				...
				// String voicemailUri = contactInfo.voicemailUri;
				// if (voicemailUri != null) {
				// intent.putExtra(CallDetailActivity.EXTRA_VOICEMAIL_URI,
				// Uri.parse(voicemailUri));
				// }
				intent.putExtra(
						CallDetailActivity.EXTRA_VOICEMAIL_START_PLAYBACK,
						false);
				if (count > 1) {
